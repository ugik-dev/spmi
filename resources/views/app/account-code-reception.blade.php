                            ...
                            <div style="min-height:50vh;" class="widget-content widget-content-area">
                                ...
                                <div class="table-responsive px-4">
                                    <table id="zero-config" class="table table-bordered">
                                        ...
                                        <tbody>
                                            @foreach ($accountCodeReceptions as $accountCode)
                                            <tr>
                                                <td style="width:40px;">{{ $loop->iteration }}</td>
                                                <td>{{ $accountCode->code ?? '-' }}</td>
                                                <td>{{ $accountCode->name }}</td>
                                                <td class="text-center ">
                                                    <button type="button" class="btn btn-sm btn-primary" onclick="openEditModal({{ $accountCode->id }}, '{{ $accountCode->name }}', '{{ $accountCode->code }}')">
                                                        <i class="text-white" data-feather="edit-2"></i>
                                                    </button>

                                                    <a href="javascript:void(0);" class="btn btn-danger btn-sm" role="button" onclick="confirmDelete({{ $accountCode->id }});">
                                                        <i class="text-white" data-feather="trash-2"></i>
                                                    </a>
                                                    <!-- Hidden form for delete request -->
                                                    <form id="delete-form-{{ $accountCode->id }}" action="{{ route('account_code_reception.delete', $accountCode->id) }}" method="POST" style="display: none;">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="{{ route('account_code_reception.store') }}" method="POST">
                                    @csrf
                            ...
                            <div class="modal-body">
                                <form id="edit-form" action="" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="account_code_name" name="name" class="form-control" required>
                                    </div>
                                    <div class="form-group mt-3">
                                        <label>Kode</label>
                                        <input type="text" id="account_code_code" name="code" class="form-control" required>
                                    </div>
